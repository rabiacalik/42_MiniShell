...
/*   Updated: 2023/07/29 17:59:33 by rcalik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// structın default degerlerini veriyor ve env degişkenlerini kopyalayıp duzenliyor
...
int	main(int ac, char **av, char **env)
{
	...
	while (1)
	{
		...
		if (g_shell.line[0] == '\0')
			continue ;
		add_history(g_shell.line);	//geçmiş komutları tutar
		parsing();
		if (heredoc_fill() != -1)
			start(0);
		myfree();
