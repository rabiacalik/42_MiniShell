...
#include "minishell.h"
/*	
	komut içerisindeki kelimeleri (| ve yönlendirmeler dahil)
	sayar ve geriye int bir değer döndürür
		"echo Hello | grep 'pattern' > output.txt"
			echo
			Hello
			|
			grep
...
/*
	bir komut satırını analiz ederek
	çift pointerlı bir diziye kayıt eder ve geriye döndürür
*/
char	**command_parser(char *command)
{
	...
	while (*command != '\0')
	{
		...
		command = command + ft_strlen(commands[index]); //şu ana kadar alınan komut sayısı kadar commandı kaydırır
		while (*command == ' ')
			command++;
		index++;
	}
